...
from thefudge import main


def test_setup_user_dir():
    with patch('thefudge.main.Path.is_dir', return_value=False), \
         patch('thefudge.main.Path.mkdir') as mkdir, \
            patch('thefudge.main.Path.touch') as touch:
        main.setup_user_dir()
        assert mkdir.call_count == 2
        assert touch.call_count == 1
    with patch('thefudge.main.Path.is_dir', return_value=True), \
         patch('thefudge.main.Path.mkdir') as mkdir, \
            patch('thefudge.main.Path.touch') as touch:
        main.setup_user_dir()
...
def test_get_settings():
    with patch('thefudge.main.load_source', return_value=Mock(rules=['bash'])):
        assert main.get_settings(Path('/')).rules == ['bash']
    with patch('thefudge.main.load_source', return_value=Mock(spec=[])):
...
def test_load_rule():
    ...
    with patch('thefudge.main.load_source',
               return_value=Mock(
                   match=match,
...
def test_get_rules():
    with patch('thefudge.main.Path.glob') as glob, \
            patch('thefudge.main.load_source',
...
def test_get_command():
    with patch('thefudge.main.Popen') as Popen:
        Popen.return_value.stdout.read.return_value = b'stdout'
        Popen.return_value.stderr.read.return_value = b'stderr'
        assert main.get_command(['thefudge', 'apt-get', 'search', 'vim']) \
